...
/* Жидкость: цвет жидкости (ргб), пока число, потом можно изменить на ргб;
...
{
    if (p <= 0) {
        throw std::invalid_argument("Ошибка: значение P должно быть > 0.");
    }
    if (v <= 0) {
        throw std::invalid_argument("Ошибка: значение V должно быть > 0.");
    }
